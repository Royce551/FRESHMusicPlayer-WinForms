...
namespace FRESHMusicPlayer.Handlers.Integrations
{
    ...
    class DiscogsIntegration : Integration
    {
        ...
        {   
            var json = JObject.Parse(InternetHandler.HttpClient.GetStringAsync($"https://api.discogs.com/database/search?q={{{query.Title}}}&{{track}}&per_page=1&key={Key}&secret={Secret}").Result);
            ...
            {                                               // Reference for json format - https://www.discogs.com/developers#page:database,header:database-search
                ...
                AlbumArt = Image.FromStream(InternetHandler.HttpClient.GetStreamAsync(json.SelectToken("results[0].cover_image").ToString()).Result),
